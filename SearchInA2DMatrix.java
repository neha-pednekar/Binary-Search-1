// Time Complexity : O(log(m*n)) for both approaches. m is the number of rows and n is the number of columns 
// Space Complexity : O(1) for both approaches as we do not use any extra space
...
public class SearchInA2DMatrix {
    ...
    */
    ...
    // Time: O(log(m*n))
    // Space: O(1)
    public boolean searchMatrix_usingSingleBinarySerach(int[][] matrix, int target) {
    ...
    // Space: O(1)
    public boolean searchMatrix_usingTwoBinarySearches(int[][] matrix, int target) {
        // We return false if matrix is null or empty
        if (matrix == null || matrix.length == 0 || matrix[0].length == 0) {
            return false;
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(searchInA2DMatrix.searchMatrix_usingSingleBinarySerach(matrix, target1));
        System.out.println(searchInA2DMatrix.searchMatrix_usingSingleBinarySerach(matrix, target2));

        System.out.println(searchInA2DMatrix.searchMatrix_usingTwoBinarySearches(matrix, target1));
        System.out.println(searchInA2DMatrix.searchMatrix_usingTwoBinarySearches(matrix, target2));
    }
}